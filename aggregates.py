...
def birth_range(x):
    if np.isnan(x.born_end):
        x.born_end = sys.maxint
    return ((ch.born_end >= x.born_start - year_gap) &
            (x.born_end + year_gap >=
            ch.born_start)).sum()
...
characters_and_alliances = pd.merge(character_df, alliances_df,
                                    on='character_id')
...
conn = engine.connect()
ch.to_sql('intersected_characters', conn, flavor=None, schema=None,
